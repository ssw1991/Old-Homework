...
print(myid, 'To split file into segments', split-Starttime,flush=True)
...
print(myid, 'To split segment into blocks', block-split, 'Total number of blocks', len(block_starts)-1,flush=True)
Proc+=(block-split)
...
for block in block_starts [0:-1]:
    ...
    print(myid, 'To read block',i,read-blocktime,flush=True)
    ...
    print(myid, 'To process block',endblock-read,flush=True)
    Proc+=(endblock-read)
...
print(myid, 'To pass statistics', pass_stat-endblock,flush=True)
if myid==0:
    ...
    if skew_p <= .05 or kurt_p <= .05:
            print('The data set does not come from a normal distribution. Its skewness is %.2f and its kurtosis is %.2f' %(skew,kurt),flush=True)
            print('The average log return is', logavg,flush=True)
            print('There were %s observations' %(n),flush=True)
            print('The standard deviation is', secondmoment/(n-1),flush=True)
    else:
            print('The data may come from a normal distribution. Its skewness is %.2f and its kurtosis is %.2f' %(skew,kurt),flush=True)
            print('The average log return is', logavg,flush=True)
            print('There were %s observations' %(n),flush=True)
            print('The standard deviation is', secondmoment/(n-1),flush=True)
    
    end=dt.datetime.now()
    print(myid, 'To compute final statistics', end-pass_stat, 'Total time',end-Starttime,flush=True)
print(myid, 'Total input time', Input,flush=True)
print(myid, 'Total output time', Output,flush=True)
print(myid,'Total process time',Proc,flush=True)
if myid==0:
    pr.disable()
